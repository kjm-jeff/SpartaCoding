 Mfrom pymongo import MongoClient  # pymongo를 임포트 하기(패키지 설치 먼저 해야겠죠?)
...
for wizard in wizard_infos:
    print(wizard)

wizard = db.users.find_one({'name': '론'})
print(wizard)

db.users.update_one({'name': '론'}, {'$set': {'age': 20}})
wizard = db.users.find_one({'name': '론'})
print(wizard)